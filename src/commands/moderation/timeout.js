...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!member) {
            return interaction.editReply({ content: `${emojis.get('error', interaction.guildId)} User not found in this server.` });
        }

        if (member.roles.highest.position >= interaction.member.roles.highest.position && interaction.user.id !== interaction.guild.ownerId) {
            return interaction.editReply({ content: `${emojis.get('error', interaction.guildId)} You cannot timeout someone with a role higher or equal to yours.` });
        }

        const durationMs = ms(durationStr);
        if (!durationMs || durationMs < 5000 || durationMs > 2.419e9) {
            return interaction.editReply({ 
                content: `${emojis.get('error', interaction.guildId)} Invalid duration. Must be between 5s and 28 days (e.g. 10m, 1h, 1d).`, 
            });
        }
        ...
        try {
            ...
            const successEmbed = new EmbedBuilder()
                .setTitle(`${emojis.get('timeout', interaction.guildId)} User Timed Out`)
            ...
            console.error(error);
            await interaction.editReply({ content: `${emojis.get('error', interaction.guildId)} Failed to timeout user. Check current status or permissions.` });
        }
    },
};
